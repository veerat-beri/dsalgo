...
class RabinKarpStringMatch:
    ...
    def __get_hash_values(self):
        ...
        for str_index in range(self.pattern_str_len):
            ...
            print(pattern_hash, text_hash_so_far, self.pattern_str[str_index], self.text_str[str_index])
    ...
    def get_pattern_occurrence_indexes(self):
        ...
        pattern_hash, text_hash_so_far = self.__get_hash_values()
        for str_index in range(max_pos_for_hash_computations + 1):



            # print(pattern_hash, text_hash_so_far)
            if pattern_hash == text_hash_so_far and self._pattern_matches(str_index):
                yield str_index
            if str_index < max_pos_for_hash_computations:
                text_hash_so_far = self.__get_updated_text_window_hash(str_index, text_hash_so_far)
