...
class LinkedBinaryTreeTraversalMixin:
    ...
    def bfs(self, node):
        ...
        # Using deque rather than normal list, as popping complexity would be more than O(1) otherwise.
        # ref: https://stackoverflow.com/a/195647
        bfs_queue = deque()  # Used as Queue
        bfs_queue.append((node, 0, 0))

        while bfs_queue:
            node, node_level, node_hd = bfs_queue.popleft()
            yield node, node_level, node_hd
            ###############
            # 1st Approach
